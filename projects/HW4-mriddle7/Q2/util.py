...
def entropy(class_y):
    ...
    if len(class_y) == 0:
        ...
        probability_1 = sum_label_1 / len(class_y)
        probability_0 = sum_label_0 / len(class_y)

    if probability_0 == 0:
        pass
    else:
        entropy += -1 * (probability_0) * np.log2(probability_0)

    if probability_1 == 0:
        pass
    else:
        entropy += -1 * (probability_1) * np.log2(probability_1)
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    y_left = [i for x, i in zip(X, y) if x[split_attribute] <= split_val]
    y_right = [i for x, i in zip(X, y) if x[split_attribute] > split_val]
...
def information_gain(previous_y, current_y):
    ...
    if len(current_y) > 0 and len(previous_y) > 0:
        ...
        for i in current_y:
            info_gain -= entropy(i) * (len(i) / ny_length)
