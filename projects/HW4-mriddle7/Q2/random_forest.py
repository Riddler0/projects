...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        sample_indices = np.random.choice(indices, n)
        samples = []
        for i in sample_indices:
            samples.append(XX[i][:-1])

        labels = [i[-1] for i in XX]  # class labels for the sampled records
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                tree1 = self.decision_trees[0]
                y = np.append(y, tree1.classify(record))
            else:
